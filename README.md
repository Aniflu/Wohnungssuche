...
| `exclude_keywords` | string[] | Post-Filter: Inserate mit einem dieser Begriffe (Titel **und** Beschreibung, Groß-/Kleinschreibung und Umlaut-Schreibweise egal) werden verworfen. Default: `wg-zimmer`, `wg zimmer`, `in wg`, `private room`, `privatzimmer`, `zwischenmiete`, `untermiete`, `befristet`, `befristung`, `nachmiete`, `auf zeit`, `zeitmiete`, `zwischenmieter`, `temporär`, `temporaer`, `sublease`, `sublet`, ` months`, `eine woche`, ` a week`, `möbliert`, `moebliert`, `furnished` – Kleinanzeigens Zimmer-/Tausch-Filter schließt WG-Zimmer und Zwischenmiete/Untermiete nicht zuverlässig aus, da sie zur selben Kategorie zählen und die Zimmerzahl der Gesamtwohnung angeben. Leere Liste `[]` deaktiviert den Filter. |
| `exclude_title_keywords` | string[] | Post-Filter nur gegen den **Titel**: verwirft Gesuche (jemand sucht eine Wohnung), die Kleinanzeigen unzuverlässig zwischen die Angebote mischt. Default: `gesuch`, `gesucht`, `suche `, `suchen `, `sucht `. Bewusst nicht gegen die Beschreibung, weil „Sie suchen …" dort häufiger Werbetext ist. Leere Liste `[]` deaktiviert den Filter. |
